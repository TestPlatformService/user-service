...
func GenerateJWT(user *pb.GetProfileResponse) *pb.LoginResponse {
        ...
        con := config.Load()
        access, err := accesstoken.SignedString([]byte(con.Token.ACCES_KEY))
        if err != nil {
        ...
        refresh, err := refreshToken.SignedString([]byte(con.Token.REFRESH_KEY))
        if err != nil {
                log.Fatalf("Error with generating access token: %s", err)
        }
...
func RefreshJWT(refreshTokenString string) (*pb.LoginResponse, error) {
        con := config.Load()
        token, err := jwt.Parse(refreshTokenString, func(token *jwt.Token) (interface{}, error) {
                if _, ok := token.Method.(*jwt.SigningMethodHMAC); !ok {
                        return nil, errors.New("unexpected signing method")
                }
                return []byte(con.Token.REFRESH_KEY), nil
        })
